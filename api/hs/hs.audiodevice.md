...
| **Notes**                                            | <ul><li>The return value will be a floating point number * This method will inspect the device to determine if it is an input or output device, and return the appropriate volume. For devices that are both input and output devices, see <code>:inputVolume()</code> and <code>:outputVolume()</code></li></ul> |
