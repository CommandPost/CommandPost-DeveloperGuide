...
| **Parameters**                                       | <ul><li>launchPath - A string containing the path to an executable file.  This must be the full path to an executable and not just an executable which is in your environment's path (e.g. <code>/bin/ls</code> rather than just <code>ls</code>).</li></ul><ul><li>callbackFn - A callback function to be called when the task terminates, or nil if no callback should be called. The function should accept three arguments:</li></ul><ul><li>exitCode - An integer containing the exit code of the process</li></ul><ul><li>stdOut - A string containing the standard output of the process</li></ul><ul><li>stdErr - A string containing the standard error output of the process</li></ul><ul><li>streamCallbackFn - A optional callback function to be called whenever the task outputs data to stdout or stderr. The function must return a boolean value - true to continue calling the streaming callback, false to stop calling it. The function should accept three arguments:</li></ul><ul><li>task - The hs.task object</li></ul><ul><li>stdOut - A string containing the standard output received since the last call to this callback</li></ul><ul><li>stdErr - A string containing the standard error output received since the last call to this callback</li></ul><ul><li>arguments - An optional table of command line argument strings for the executable</li></ul>   |
| **Returns**                                          | <ul><li>An <code>hs.task</code> object</li></ul>            |
| **Notes**                                            | <ul><li>The arguments are not processed via a shell, so you do not need to do any quoting or escaping. They are passed to the executable exactly as provided.</li></ul>                 |
...
| **Notes**                                            | <ul><li>This should only be called on tasks with a streaming callback - tasks without it will automatically close stdin when any data supplied via <code>hs.task:setInput()</code> has been written</li></ul><ul><li>This is primarily useful for sending EOF to long-running tasks</li></ul>                 |
...
| **Notes**                                            | <ul><li>If the task is not paused, the error message will be printed to the Hammerspoon Console</li></ul><ul><li>This method can be called multiple times, but a matching number of <code>hs.task:resume()</code> calls will be required to allow the process to continue</li></ul>                 |
...
| **Returns**                                          | <ul><li>The hs.task object</li></ul>            |
| **Notes**                                            | <ul><li>For information about the requirements of the callback function, see <code>hs.task.new()</code></li></ul><ul><li>If a callback is removed without it previously having returned false, any further stdout/stderr output from the task will be silently discarded</li></ul>                 |
...
| **Notes**                                            | <ul><li>You can only set the working directory if the task has not already been started.</li></ul><ul><li>This will only set the directory that the task starts in.  The task itself can change the directory while it is running.</li></ul>                 |
