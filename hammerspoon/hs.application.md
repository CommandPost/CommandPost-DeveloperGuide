...
| Parameters |  * pid - a UNIX process id (i.e. a number) | | Returns |  * An hs.application object if one can be found, otherwise nil | 
#### [applicationsForBundleID](#applicationsForBundleID)
| | |
|-|-|
...
| Parameters |  * bundleID - An OSX application bundle indentifier | | Returns |  * A table of zero or more hs.application objects that match the given identifier | 
#### [enableSpotlightForNameSearches](#enableSpotlightForNameSearches)
| | |
|-|-|
...
| Parameters |  * `state` - an optional boolean specifying whether or not Spotlight should be used to try and determine alternate application names for `hs.application.find` and similar functions. | | Returns |  * the current, possibly changed, state | | Notes |  * This setting is persistent across reloading and restarting Hammerspoon. * If this was set to true and you set it to true again, it will purge the alternate name map and rebuild it from scratch. * You can disable Spotlight alternate name mapping by setting this value to false or nil. If you set this to false, then the notifications indicating that more results might be possible if Spotlight is enabled will be suppressed. | 
#### [frontmostApplication](#frontmostApplication)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * An hs.application object | 
#### [infoForBundleID](#infoForBundleID)
| | |
|-|-|
...
| Parameters |  * bundleID - A string containing an application bundle identifier (e.g. "com.apple.Safari") | | Returns |  * A table containing information about the application, or nil if the bundle identifier could not be located | 
#### [launchOrFocus](#launchOrFocus)
| | |
|-|-|
...
| Parameters |  * bundleID - A string containing the bundle ID of the application to either launch or focus. | | Returns |  * True if the application was either launched or focused, otherwise false (e.g. if the application doesn't exist) | | Notes |  * Bundle identifiers typically take the form of `com.company.ApplicationName` | 
#### [nameForBundleID](#nameForBundleID)
| | |
|-|-|
...
| Parameters |  * bundleID - A string containing an application bundle identifier (e.g. "com.apple.Safari") | | Returns |  * A string containing the application name, or nil if the bundle identifier could not be located | 
#### [pathForBundleID](#pathForBundleID)
| | |
|-|-|
...
| Parameters |  * bundleID - A string containing an application bundle identifier (e.g. "com.apple.Safari") | | Returns |  * A string containing the app bundle's filesystem path, or nil if the bundle identifier could not be located | 
#### [runningApplications](#runningApplications)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A table containing zero or more hs.application objects currently running on the system | 
### Constructors
...
| Parameters |  * hint - search criterion for the desired application; it can be:   - a pid number as per `hs.application:pid()`   - a bundle ID string as per `hs.application:bundleID()`   - an application name string as per `hs.application:name()` | | Returns |  * an hs.application object for a running application that matches the supplied search criterion, or `nil` if not found | | Notes |  * see also `hs.application.find` | 
#### [open](#open)
| | |
|-|-|
...
| Parameters |  * allWindows - If true, all windows of the application will be brought to the front. Otherwise, only the application's key window will. Defaults to false. | | Returns |  * A boolean value indicating whether or not the application could be activated | 
#### [allWindows](#allWindows)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A table of zero or more hs.window objects owned by the application | | Notes |  * This function can only return windows in the current Mission Control Space; if you need to address windows across   different Spaces you can use the `hs.window.filter` module   - if `Displays have separate Spaces` is *on* (in System Preferences>Mission Control) the current Space is defined     as the union of all currently visible Spaces   - minimized windows and hidden windows (i.e. belonging to hidden apps, e.g. via cmd-h) are always considered     to be in the current Space | 
#### [bundleID](#bundleID)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A string containing the bundle identifier of the application | 
#### [findMenuItem](#findMenuItem)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * An hs.window object representing the window of the application that currently has focus, or nil if there are none | 
#### [getMenuItems](#getMenuItems)
| | |
|-|-|
...
| Parameters |  * title - the desired window's title string as per `hs.window:title()` | | Returns |  * the desired hs.window object belonging to this application, or `nil` if not found | 
#### [hide](#hide)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A boolean indicating whether the application was successfully hidden | 
#### [isFrontmost](#isFrontmost)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * True if the application is the frontmost application, otherwise false | 
#### [isHidden](#isHidden)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A boolean indicating whether the application is hidden or not | 
#### [isRunning](#isRunning)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A boolean, true if the application is running, false if not | | Notes |  * If an application is terminated and re-launched, this method will still return false, as `hs.application` objects are tied to a specific instance of an application (i.e. its PID) | 
#### [kill](#kill)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A number that is either 1 if the app is in the dock, 0 if it is not, or -1 if the application is prohibited from having GUI elements | 
#### [mainWindow](#mainWindow)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * An hs.window object representing the main window of the application, or nil if it has no windows | 
#### [name](#name)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A string containing the filesystem path of the application | 
#### [pid](#pid)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * The UNIX process identifier of the application (i.e. a number) | 
#### [selectMenuItem](#selectMenuItem)
| | |
|-|-|
...
| Parameters |  * menuitem - The menu item to select, specified as either a string or a table. See the `menuitem` parameter of `hs.application:findMenuItem()` for more information. * isRegex - An optional boolean, defaulting to false, which is only used if `menuItem` is a string. If set to true, `menuItem` will be treated as a regular expression rather than a strict string to match against | | Returns |  * True if the menu item was found and selected, or nil if it wasn't (e.g. because the menu item couldn't be found) | | Notes |  * Depending on the type of menu item involved, this will either activate or tick/untick the menu item | 
#### [title](#title)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A string containing the name of the application | 
#### [unhide](#unhide)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A boolean indicating whether the application was successfully unhidden | 
#### [visibleWindows](#visibleWindows)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * A table containing zero or more hs.window objects | 