...
| Parameters |  * None | | Returns |  * True if Location Services are enabled, otherwise false | 
#### [start](#start)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * True if the operation succeeded, otherwise false | | Notes |  * This function activates Location Services for Hammerspoon, so the first time you call this, you may be prompted to authorise Hammerspoon to use Location Services. | 
#### [stop](#stop)
| | |
|-|-|
...
| Parameters |  * `tag` - A string containing the unique tag a callback was registered with | | Returns |  * None | 
### Constructors
...
| Parameters |  * None | | Returns |  * a locationObject | | Notes |  * The locationObject created will receive callbacks independant of all other locationObjects and the legacy callback functions created with [hs.location.register](#register).  It can also receive callbacks for region changes which are not available through the legacy callback mechanism. | 
### Methods
...
| Parameters |  * `regionTable` - a region table as described in the module header | | Returns |  * if the region table was able to be added to Location Services for monitoring, returns the locationObject; otherwise returns nil | | Notes |  * This method activates Location Services for Hammerspoon, so the first time you call this, you may be prompted to authorise Hammerspoon to use Location Services. * If the `identifier` key is not provided, a new UUID string is generated and used as the identifier. * If the `identifier` key matches an already monitored region, this region will replace the existing one. | 
#### [callback](#callback)
| | |
|-|-|
...
| Parameters |  * `identifier` - a string which should contain the identifier of the region to remove from monitoring | | Returns |  * if the region identifier matches a currently monitored region, returns the locationObject; if it does not match a currently monitored region, returns false; returns nil if an error occurs or if Location Services is not currently active (no function or method which activates Location Services has been invoked yet) or enabled for Hammerspoon. | | Notes |  * This method activates Location Services for Hammerspoon, so the first time you call this, you may be prompted to authorise Hammerspoon to use Location Services. * If the `identifier` key is not provided, a new UUID string is generated and used as the identifier. * If the `identifier` key matches an already monitored region, this region will replace the existing one. | 
#### [startTracking](#startTracking)
| | |
|-|-|
...
| Type        | Method |
| Description | Enable callbacks for location changes/refinements for this locationObject |
| Parameters |  * None | | Returns |  * the locationObject | | Notes |  * This function activates Location Services for Hammerspoon, so the first time you call this, you may be prompted to authorise Hammerspoon to use Location Services. | 
#### [stopTracking](#stopTracking)
| | |
|-|-|
...
| Parameters |  * None | | Returns |  * the locationObject | 