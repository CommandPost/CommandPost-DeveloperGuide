...
| Parameters |  * None | | Returns |  * a string containing the working directory for the task. | | Notes |  * This only returns the directory that the task starts in.  If the task changes the directory itself, this value will not reflect that change. | 